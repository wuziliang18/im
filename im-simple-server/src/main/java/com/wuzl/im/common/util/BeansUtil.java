...
/**
 * 类BeansUtil.java的实现描述：TODO 类实现描述
 * 
 * @author ziliang.wu 2017年4月7日 上午11:03:49
 */
public class BeansUtil {
    ...
    public static Map<String, Object> convertBeanToMap(Object bean) {
        ...
        try {
            ...
            e.printStackTrace();
            throw new RuntimeException("bean[" + bean.getClass().getName() + "]转化为map时出错");
        }
        /* 6.返回结果 */
        return returnMap;
    }
    ...
    public static void copyBean(Object fromObj, Object toObj) {
        ...
        try {
            ...
            e.printStackTrace();
            throw new RuntimeException("bean[" + fromObj.getClass().getName() + "]复制时出错");
        }
    }
     ...
     * @param cover是否覆盖
     */
    public static void convertBeanToMapIgnoreFormat(Map<String, Object> dto, Object bean, boolean cover) {
        if (dto == null) {
     ...
     * @return
     */
    public static <T> T convertMapToBeanIgnoreFormat(Map<String, Object> srcDto, Class<T> objClass) {
        T obj;
        ...
        try {
            ...
            for (PropertyDescriptor property : propertyDescriptors) {
                ...
                if (!key.equals("class")) {
                    ...
                    if (value != null) {
                        try {
                            setter.invoke(obj, getObejctFromTypeAndValue(value, cl));
                        } catch (Exception e) {
                            e.printStackTrace();
                        }
                    } else {
                        value = dto.get(CodeFormat.delUnderlineHumpAndToLower(key));
                        if (value != null) {
                            try {
                                setter.invoke(obj, getObejctFromTypeAndValue(value, cl));
                            } catch (Exception e) {
     ...
     * @return
     */
    public static Map<String, Object> copyMapKeyDelUnderlineHumpAndToLower(Map<String, Object> dto) {
        Map<String, Object> resultMap = new HashMap<String, Object>();
        for (Map.Entry<String, Object> entry : dto.entrySet()) {
            resultMap.put(CodeFormat.delUnderlineHumpAndToLower(entry.getKey()), entry.getValue());
        }
        return resultMap;
    }
    ...
    public static boolean isBasicType(Class<?> c) {
        if (c == int.class || Integer.class.equals(c)) return true;
        else if (c == boolean.class || Boolean.class.equals(c)) return true;
        else if (c == long.class || Long.class.equals(c)) return true;
        else if (c == float.class || Float.class.equals(c)) return true;
        else if (c == double.class || Double.class.equals(c)) return true;
        else if (c == char.class) return true;
        else if (c == byte.class || Byte.class.equals(c)) return true;
        else if (c == short.class || Short.class.equals(c)) return true;
