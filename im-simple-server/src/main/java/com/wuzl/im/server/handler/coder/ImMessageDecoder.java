...
/**
 * 类ImMessageDecoder.java的实现描述：解码请求的文本数据
 * 
 * @author ziliang.wu 2017年3月13日 下午2:47:11
 */
public class ImMessageDecoder extends ByteToMessageDecoder {
    ...
    protected void decode(ChannelHandlerContext ctx, ByteBuf buffer, List<Object> out) throws Exception {
        ...
        if (requestHeader.isEncrypt()) {
            ...
            if (secretKey == null) {
                infoLogger.info("没有对应的公钥和私钥，关闭连接，对方ip：" + getRemoteIp(ctx));
                ctx.channel().close();
                return;
            }
            bodyBytes = AesUtil.decrypt(bodyBytes, secretKey);
        } else {
            if (messageType != Message.Type.PUBLIC_KEY_REQUEST) {
